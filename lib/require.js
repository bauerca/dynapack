(function(main, mainChunks, prefix) {
...
function loadChunk(newModules, newEntries) {
  var id;
...
// window['dynapackChunks'] object, which is created by the first downloaded
// chunk.
...
// Asynchronously require the main module. It will run when all of
// the chunks needed by main are loaded. The chunks needed by main
// should appear immediately after this IIFE.
pendingAsyncRequires[main] = [function(){}];
entries[main] = mainChunks;
})//(MAIN ENTRY MODULE, CHUNKS NEEDED BY MAIN, PREFIX FOR CHUNK SCRIPTS);
