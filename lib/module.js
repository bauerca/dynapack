...
function Module(opts) {
  ...
  //this._watcher = fs.watch(opts.id);
  //this._watcher.on('change', this.handleChange.bind(this));
...
Module.prototype.loadSource = function(callback) {
  ...
  if (typeof this.source === 'string') {
    setImmediate(callback.bind(null, null, this.source));
  }
  else if (this.source) {
    ...
    this.source.once('error', handleError);
    this.source.pipe(concatStream);
  }
  else {
    // Read it from the file.
    fs.readFile(this.id, {encoding: 'utf8'}, function(err, source) {
  ...
  function handleError(err) {
    err.message = 'Module could not load from stream source. ' + err.message;
    module.source = null;
    callback(err);
  }
};
...
Module.prototype.isDynamicDependencyOf = function(parent) {
  return values(parent.deps.dynamic).indexOf(this.id) >= 0;
};
